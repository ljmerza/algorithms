...
function LfuDoublyLinkedList() {
    this.head = new LfuNode('budder head', null);
    this.tail = new LfuNode('budder tail', null);
...
LfuCache.prototype.set = function (key, value) {
    ...
    if (node == undefined) {
        ...
        // if we have space for node then try to add it to linked list with freq 1
        if (this.size !== this.capacity) {

            // if linked list for freq 1 doesnt exist then create it
            if (this.freq[1] == undefined) this.freq[1] = new LfuDoublyLinkedList();

            // add new node and increment size of freq 1
            this.freq[1].insertAtHead(node);
            this.size++;

        } else {
            // else freq 1 is full and we need to delete a node first so delete tail
            const oldTail = this.freq[this.minFreq].removeAtTail();
            delete this.keys[oldTail.key];

            // if we deleted freq 1 then add it back before adding new node
            if (this.freq[1] === undefined) this.freq[1] = new LfuDoublyLinkedList();
            this.freq[1].insertAtHead(node);
        }

        // we added a new node so minFreq needs to be reset to 1 
        ...
        // save the old freq of the node and increment (also update data)
        ...
        // if the node we incremented was in the minFreq list of all lists and there's nothing left in the old list
        // then we know the new minFreq for any node in any list is in the next freq so increment that now
        if (oldFreqCount === this.minFreq && Object.keys(this.freq[oldFeqCount]).size === 0) this.minFreq++;
    }
}

/**
 * if element doesnt exist in cache return null
 * otherwaise we increment freq for that element - item is brought to head of linked list
 * min freq is adjusted if needed
 */
LfuCache.prototype.get = function (key) {
    ...
    // remove node from old freq list and create new one if next one doesnt exist
    // before adding the node to the next list at the head
    this.freq[oldFreqCount].removeNode(node);
    if (this.freq[node.freqCount] === undefined) this.freq[node.freqCount] = new LfuDoublyLinkedList();
    this.freq[node.freqCount].insertAtHead(node);

    // if old freq list is empty then update minFreq
